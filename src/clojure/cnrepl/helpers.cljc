...
(defn load-file-command
  ...
  ([f source-root]
   #?(:clj
      ...
      :cljr
      (let [^String abspath (if (string? f) f (.DirectoryName ^FileInfo f))
            source-root (cond
                          (nil? source-root) ""
                          (string? source-root) source-root
                          (instance? FileInfo source-root) (.DirectoryName ^FileInfo source-root))]
        (load-file-command (slurp abspath :encoding "UTF-8")
                           (if (and (seq source-root)
                                    (.StartsWith abspath source-root))
                             (-> abspath
