...
#?(:clj
   (defmacro ^{:author "Colin Jones"} set-signal-handler! ;; Does not make sense for .Net.  We will deal with this in a different way below
     [signal f]
...
(defn- run-repl
  ...
  ([host port {:keys [prompt err out value transport]
   ...
   (let [transport (nrepl/connect :host host :port port :transport-fn transport)
         client (nrepl/client transport #?(:clj Long/MAX_VALUE :cljr Int64/MaxValue))]
     (println (repl-intro))
     ...
     #?(:clj (Thread/sleep 50) :cljr (System.Threading.Thread/Sleep 50))
     (let [session (nrepl/client-session client)
       ...
       (loop []
         ...
         (let [input (read *in* false 'exit)]
           (if (done? input)
             #?(:clj (System/exit 0) :cljr (Environment/Exit 0))
...
(defn- ->int [x]
  (cond
    (nil? x) x
    (number? x) x
    :else #?(:clj (Integer/parseInt x) :cljr (Int32/Parse x))))
...
(defn server-started-message
  ...
  (let [transport (:transport options)
        port (:port server)
        #?@(:clj [^java.net.ServerSocket ssocket (:server-socket server)]
            :cljr [^System.Net.Sockets.Socket ssocket (:server-socket server)])
        host #?(:clj (.getHostName (.getInetAddress ssocket))
                :cljr (-> ssocket .LocalEndPoint (.System.Net.IPEndPoint port) .Address .System.Net.Dns.GetHostEntry .HostName))]
...
(defn save-port-file
  ...
  (let [port (:port server)
        port-file #?(:clj (io/file ".nrepl-port") :cljr (System.IO.FileInfo. ".nrepl-port"))]
    #?(:clj (.deleteOnExit port-file)) ;; commented out -- no easy equivalent
    (spit port-file port)))
  ...
  (cond (:help options)    (display-help)
        ...
        :else (let [options (server-opts options)
                ...
                (if (:interactive options)
                  ...
                  #?(:clj (Thread/sleep Long/MAX_VALUE)
                     :cljr (System.Threading.Thread/Sleep Int64/MaxValue))))))

(defn -main
  [& args]
  (try
    #?(:clj (set-signal-handler! "INT" handle-interrupt)) ;; TODO:  DO this with catching cancel on the console.
