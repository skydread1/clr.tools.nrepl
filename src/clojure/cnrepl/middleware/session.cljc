  ...
  (:import
   #?@(:clj
       ...
       [(clojure.lang Compiler+CompilerException LineNumberingTextReader)
        ...
        (System.Threading ThreadStart Thread ThreadState ThreadInterruptedException)])))
...
#?(:clj (defn- configure-thread-factory
...
#?(:clj (def default-executor "Delay containing the default Executor." (delay (configure-executor))))
  ...
  #?(:clj
     (let [input-queue   (LinkedBlockingQueue.)
       ...
       {:input-queue  input-queue
        :stdin-reader reader})
     :cljr
     (let [input-queue   (|System.Collections.Concurrent.BlockingCollection`1[System.Object]|.)
           request-input (fn []
...
(defn- interrupt-stop
  ...
  #?@(:clj
      ...
      [(.Interrupt t)
       ...
       (future
         (Thread/Sleep 5000)
         (when-not (= ThreadState/Stopped
                      (.ThreadState t))
           (.Suspend t)))])) ;;; would call abort, but that is not supported in .Net 5.
...
(defn session-exec
  ...
  (let [cl #?(:clj (clojure.lang.DynamicClassLoader. (.getContextClassLoader (Thread/currentThread)))
              :cljr nil)
        ...
        main-loop #?(:clj
                     ...
                     :cljr #(try
                              (loop []
                                (let [[exec-id ^System.Threading.WaitCallback r ^System.Threading.WaitCallback ack] (.Take queue)]
                                  (reset! running exec-id)
                                  (when (try
                                          ...
                                          (finally
                                            (compare-and-set! running exec-id nil)))
                                    (some-> ack System.Threading.ThreadPool/QueueUserWorkItem)
                                    (recur))))
                              (catch ThreadInterruptedException e)))
        spawn-thread #?(:clj
                        #(doto (Thread. main-loop (str "nREPL-session-" id))
                           (.setDaemon true)
                           (.setContextClassLoader cl)
                           .start)
                        :cljr
                        #(doto (Thread. (gen-delegate ThreadStart [] (main-loop)))
                                 (.set_IsBackground true)
                                 (.set_Name  (str "nREPL-session-" id))
                                 .Start))]
    ...
    {:interrupt (fn [exec-id]
     ...
     :exec (fn [exec-id r ack]
             #?(:clj (.put queue [exec-id r ack])
                :cljr (.Add queue [exec-id r ack])))}))
...
(defn add-stdin
  ...
  (fn [{:keys [op stdin session transport] :as msg}]
    (cond
      (= op "eval")
      (let [in #?(:clj (-> (meta session) ^LineNumberingPushbackReader (:stdin-reader))
                  :cljr (-> (meta session) ^LineNumberingTextReader (:stdin-reader)))]
        (binding [*skipping-eol* true]
          (clojure.main/skip-if-eol in))
        (h msg))
      (= op "stdin")
      (let [q #?(:clj (-> (meta session) ^LinkedBlockingQueue (:input-queue))
                 :cljr (-> (meta session) (:input-queue)))] ;; ^|System.Collections.Concurrent.BlockingCollection`1[System.Object]| makes the reader cond fail in clj
        (if (empty? stdin)
          #?(:clj (.put q -1) :cljr (.Add q -1))
          (locking q
            (doseq [c stdin] #?(:clj (.put q c) :cljr (.Add q c)))))
        (t/send transport (response-for msg :status :done)))
      :else
      (h msg))))
