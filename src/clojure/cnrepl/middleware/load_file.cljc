...
(defn- load-large-file-code
  ...
  (let [t #?(:clj (System/currentTimeMillis) :cljr (Environment/TickCount))
    ...
    (swap! file-contents
           (fn [file-contents]
             (let [expired-keys
                   (filter
                    (comp #(and %
                             #?(:clj (< 10000 (- (System/currentTimeMillis) %))
                                :cljr (< 100000000 (- (Environment/TickCount) %)))) ;; need to switch to ticks
                          :t meta)
                    (keys file-contents))]
    ...
    (binding [*print-length* nil
      ...
      (pr-str `(try
                 (clojure.lang.Compiler/load
                  (#?(:clj java.io.StringReader. :cljr System.IO.StringReader.)
...
(defn ^{:dynamic true} load-file-code
  ...
  [file file-path file-name]
  (apply format
         #?(:clj "(clojure.lang.Compiler/load (java.io.StringReader. %s) %s %s)"
            :cljr "(clojure.lang.Compiler/load (System.IO.StringReader. %s) nil %s %s)")
	(map (fn [item]
                (binding [*print-length* nil
                          *print-level* nil]
                  (pr-str item)))
