(ns cnrepl.middleware.print
  ...
  (:require
   ...
   #?(:cljr [cnrepl.debug :as debug]))
  (:import
   #?@(:clj
       [(java.io BufferedWriter PrintWriter StringWriter Writer)
        (nrepl QuotaExceeded) ;; custom java class
        (cnrepl.transport Transport)]
       :cljr
       [(System.IO TextWriter StringWriter StreamWriter BufferedStream)
        #_(nrepl QuotaExceeded) ;; class not introduced in .net
        (cnrepl.transport Transport)])))

;; private in clojure.core
(defn- pr-on
  [x w]
  #_(:cljr (debug/prn-thread "pr-on says:" x w   "****"))
  (if *print-dup*
    (print-dup x w)
    (print-method x w))
  nil)
...
(defn- to-char-array
  ...
  [x]
  (cond
    (string? x) #?(:clj (.toCharArray ^String x) :cljr (.ToCharArray ^String x))
     ...
     (if-not quota
       ...
       (let [total (volatile! 0)]
         (proxy [Writer] []
           ...
           (close []
             (.close writer))))))
   :cljr (defn with-quota-writer
           ^System.IO.TextWriter
           [^TextWriter writer quota]
           (if-not quota
             writer
             (let [total (volatile! 0)]
               (proxy [TextWriter] []
                 ...
                 (Write
                   ...
                   ([^|System.Char[]| x off len]
                      (locking total
                        (let [cbuf (to-char-array x)
                              rem (- quota @total)]
			                    #_(debug/prn-thread "wgw-xol" this "///"  x  off len "///" cbuf "****")
                          (vswap! total + len)
                          (.Write writer cbuf ^int off ^int (min len rem))
                          (when (neg? (- rem len))
...
#?(:clj
  ...
  transport of `msg`, keyed by `key`."
     ...
     #_(-> (proxy [TextWriter] []
           ...
           (Flush [])
           (Close []))
         #_(BufferedStream. (or buffer-size 1024))
         (with-quota-writer quota)
         #_(StreamWriter. true))))
                    ...
                    (let [value (get resp key)]
                      (try
                        (with-open [writer (replying-PrintWriter key msg opts)]
						              #_(:cljr (debug/prn-thread "S-S1: " value print-fn writer))
						              #_(:cljr (debug/prn-thread "S-S2: " value))
                          (print-fn value writer))
                        (catch #?(:clj QuotaExceeded :cljr ArgumentOutOfRangeException) _
                          (transport/send
                           transport
...
(defn- send-nonstreamed
  ...
  (let [print-key (fn [key]
                    (let [value (get resp key)
                      ...
                      (try
						            #_(:cljr (debug/prn-thread "S-NS1: " value print-fn writer))
                        (print-fn value writer)
                        (catch #?(:clj QuotaExceeded :cljr ArgumentOutOfRangeException) _
  ...
  (reify Transport
    ...
    (send [this resp]
      (let [{:keys [::stream?] :as opts} (-> (merge msg (bound-configuration) resp opts)
        ...
        (if stream?
          (do #_(:cljr (debug/prn-thread "send-streamed" msg resp opts)) (send-streamed msg resp opts))
          (do #_(:cljr (debug/prn-thread "send-nonstreamed" msg resp opts)) (send-nonstreamed msg resp opts))))
      this)))
  ...
  * `::keys` – a seq of the keys in the response whose values should be printed.
  ...
  (fn [{:keys [::options] :as msg}]
    (let [print-var (resolve-print msg)
          print (fn [value writer]
                  #_(:cljr (debug/prn-thread "wp " print-var value writer))
                  (if print-var
