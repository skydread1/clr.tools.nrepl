(ns cnrepl.core-test
  (:require
   ...
   [cnrepl.test-helper :refer [platform-newlines]]
   [clojure.main]
   [clojure.set :as set]
  ...
  (:import
   #?@(:clj
       ...
       :cljr
       [(System.Threading Thread) (System.IO FileInfo)
  ...
  `(deftest ~name
     (with-open [transport# (connect :port (:port *server*)
       ...
       (let [~'transport transport#
             ~'client (client transport# #?(:clj Long/MAX_VALUE :cljr Int32/MaxValue))
...
#?(:clj
   (def-repl-test session-*out*-writer-length-translation ;DM: TODO - figure this one out
...
(def-repl-test exceptions
  (let [{:keys [status err value]} (-> session
    ...
    (is (nil? value))
    (is (#?(:clj .contains :cljr .Contains) err "bad, bad code"))
    (is (= [true] (repl-values session #?(:clj "(.contains (str *e) \"bad, bad code\")"
                                          :cljr "(.Contains (str *e) \"bad, bad code\")"))))))
...
(def-repl-test interrupt
  ...
  (testing "registered session"
    ...
    (let [resp (message session {:op "eval" :code (code (do
                                                          (def halted? true)
                                                          halted?
                                                          #?(:clj (Thread/sleep 30000)
                                                             :cljr (Thread/Sleep 30000))
                                                          (def halted? false)))})]
      #?(:clj (Thread/sleep 100)
         :cljr (Thread/Sleep 100))
  ...
  (testing "interrupting a sleep"
    (let [resp (message session {:op "eval"
                                 :code (code
                                        (do
                                          #?(:clj (Thread/sleep 10000)
                                             :cljr (Thread/Sleep 10000))
                                          "Done"))})]
      #?(:clj (Thread/sleep 100) :cljr (Thread/Sleep 100))
      (is (= #{:done}
             (->> session
      ...
      (let [r (->> resp
        ...
        (is (= "class java.lang.InterruptedException"
               (:ex r))))))
  #?(:clj
     (testing "interruptable code" ;;; TODO: HOw?
       (let [resp (message session {:op "eval"
                                    :code (code
                                           (do
                                             (defn run []
...
(def-repl-test read-timeout
  (is (nil? (repl-values timeout-session #?(:clj "(Thread/sleep 1100) :ok"
                                            :cljr "(Thread/Sleep 1100) :ok"))))
  ...
  (testing "multiple messages can be handled on the same connection concurrently"
    (let [sessions (doall (repeatedly 3 #(client-session client)))
          ...
          elapsed-times (map (fn [session eval-duration]
                               (let [expr #?(:clj (pr-str `(Thread/sleep ~eval-duration))
                                             :cljr (pr-str `(Thread/Sleep ~eval-duration)))
                                     responses (message session {:op "eval" :code expr})]
                                 (future
                                   (is (= [nil] (response-values responses)))
                                   #?(:clj (- (System/currentTimeMillis) start-time)
                                      :cljr (- (Environment/TickCount) start-time)))))
                             sessions
                             [2000 1000 0])]
...
#?(:clj
   ...
   (def-repl-test ensure-transport-closeable
     ...
     (.Dispose transport)
     (is (thrown? ObjectDisposedException (repl-values session "5")))))
...
(defn- disconnection-exception?
  ...
  #?(:clj
     ...
     :cljr
     (or (and (instance? ObjectDisposedException (root-cause e))
              (re-find #"Cannot access.*" (.Message (root-cause e))))
         (and (instance? SocketException (root-cause e))
              (re-find #".*closed.*" (.Message (root-cause e)))))))
  ...
  (testing "Ensure that transports fail ASAP when the server they're connected to goes down."
    (let [server (server/start-server :transport-fn *transport-fn*)
      ...
      (let [reader (future (while true (transport/recv transport)))]
        #?(:clj (Thread/sleep 1000) :cljr (Thread/Sleep 1000))
        #?(:clj (.close server) :cljr (.Dispose server))
        #?(:clj (Thread/sleep 1000) :cljr (Thread/Sleep 1000))
        (try
      ...
      (is (thrown? #?(:clj SocketException :cljr ObjectDisposedException) (transport/recv transport)))
      ...
      #?(:clj (Thread/sleep 1000) :cljr (Thread/Sleep 1000))
      (is (thrown? #?(:clj SocketException :cljr ObjectDisposedException) (transport/send transport {"op" "eval" "code" "(+ 5 1)"}))))))

(deftest server-starts-with-minimal-configuration
  (testing "Ensure server starts with minimal configuration"
    (let [server (server/start-server)
          transport (connect :port (:port server))
          client (client transport #?(:clj Long/MAX_VALUE :cljr Int64/MaxValue))]
      (is (= ["3"]
...
(def-repl-test clients-fail-on-disconnects
  (testing "Ensure that clients fail ASAP when the server they're connected to goes down."
    ...
    (try (repl-eval session "(+ 1 1)") (catch #?(:clj Throwable :cljr Exception) _))
    #?(:clj (Thread/sleep 100) :cljr (Thread/Sleep 100))
    (is (thrown? #?(:clj SocketException :cljr ObjectDisposedException) (repl-eval session "(+ 1 1)")))))
...
(def-repl-test agent-await
  (is (= [42] (repl-values session (code (let [a (agent nil)]
                                           (send a (fn [_] #?(:clj (Thread/sleep 1000)
                                                              :cljr (System.Threading.Thread/Sleep 1000)) 42))
                                           (await a)
                                           @a))))))
...
(def-repl-test interrupt-load-file
  (let [resp (message session {:op "load-file"
                               :file (slurp (#?(:clj File.
                                                :cljr System.IO.Path/Combine)
                                             project-base-dir
                                             "load-file-test/nrepl/load_file_sample2.clj"))
...
(def dynamic-test-lookup
  {["resource" "foo.clj"]
   ...
   ["resource" "ping.clj"]
   (slurp
    #?(:clj (File. project-base-dir "test-resources/ping.clj")
       :cljr (System.IO.Path/Combine project-base-dir "test-resources/ping.clj")))

   ["resource" "ping_imp.clj"]
   (slurp
    #?(:clj (File. project-base-dir "test-resources/ping_imp.clj")
       :cljr (System.IO.Path/Combine project-base-dir "test-resources/ping_imp.clj")))})
